...
describe("$boolean undefined", function()
  it("returns undefined on undefined (not false)", function()
    assert.is_nil(run("$boolean(nope)", {}))
    assert.is_nil(run("$boolean(foo.bar)", { foo = {} }))
  end)

  it("still coerces normal values", function()
    assert.is_true(run("$boolean(1)"))
    assert.is_false(run("$boolean(0)"))
    assert.is_true(run('$boolean("x")'))
    assert.is_false(run("$boolean([])"))
  end)
end)
