...
describe("explain: public jsonata.explain", function()
  local jsonata = require("jsonata")

  it("tokens stage", function()
    local out = jsonata.explain("nums[$>2]", nil, "tokens")
    assert.is_truthy(out:find("== TOKENS =="))
    assert.is_truthy(out:find('"nums"'))
  end)

  it("ast vs ast-norm differ for predicates", function()
    local raw = jsonata.explain("nums[$>2]", nil, "ast")
    local norm = jsonata.explain("nums[$>2]", nil, "ast-norm")
    assert.is_truthy(raw:find("predicate"))
    assert.is_truthy(norm:find("path"))
  end)

  it("eval stage traces", function()
    local out = jsonata.explain("nums[$>2]", { nums = { 1, 2, 3, 4 } }, "eval")
    assert.is_truthy(out:find("== EVAL =="))
    assert.is_truthy(out:find("3, 4"))
  end)

  it("all stage (default) has four sections", function()
    local out = jsonata.explain("nums[$>2]", { nums = { 1, 2, 3, 4 } })
    assert.is_truthy(out:find("== TOKENS =="))
    assert.is_truthy(out:find("== AST %(raw"))
    assert.is_truthy(out:find("== AST %(normalized"))
    assert.is_truthy(out:find("== EVAL =="))
  end)

  it("renders parse errors instead of throwing", function()
    local out = jsonata.explain("a[", nil, "ast")
    assert.is_truthy(out:find("error"))
  end)
end)
