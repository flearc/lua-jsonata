...
describe("tokenizer", function()
  ...
  it("skips block comments like whitespace", function()
    local t = tokens("1 /* plus one */ + 1")
    assert.are.equal("number", t[1].type)
    assert.are.equal(1, t[1].value)
    assert.are.equal("+", t[2].value)
    assert.are.equal(1, t[3].value)
    assert.is_nil(t[4])
  end)

  it("tokenizes boolean operator words as names when an operand is expected", function()
    local t = tokens("and=1 and or=2")
    assert.are.equal("name", t[1].type)
    assert.are.equal("and", t[1].value)
    assert.are.equal("keyword", t[4].type)
    assert.are.equal("and", t[4].value)
    assert.are.equal("name", t[5].type)
    assert.are.equal("or", t[5].value)
  end)
...
describe("tokenizer: range .. and number fix (M2d)", function()
  it("tokenizes 1..5 as number .. number (no longer swallows the dot)", function()
    local t = tokens("1..5")
    assert.are.equal(3, #t)
    assert.are.equal("number", t[1].type)
    assert.are.equal(1, t[1].value)
    assert.are.equal("operator", t[2].type)
    assert.are.equal("..", t[2].value)
    assert.are.equal("number", t[3].type)
    assert.are.equal(5, t[3].value)
  end)

  it("still tokenizes decimals and exponents", function()
    assert.are.equal(5.5, tokens("5.5")[1].value)
    assert.are.equal(1500, tokens("1.5e3")[1].value)
  end)
end)
